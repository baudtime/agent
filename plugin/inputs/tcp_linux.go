...
const (
	tcpConnNumMetric     = "tcp_conn_num"
	tcpRetransMetric     = "tcp_retrans"
	tcpActiveOpensMetric = "tcp_active_opens"
)

func init() {
	register("tcp", plugin.DefaultDisabled, newTCPCollector)
}

type tcpCollector struct {
	lastTcpRetransSegs uint64
	lastTcpActiveOpens uint64
	lastCollectTime    time.Time
}

func newTCPCollector() (plugin.Input, error) {
	return &tcpCollector{}, nil
}

func (c *tcpCollector) Collect(ch chan<- plugin.Metric) error {
	snmp, err := readSnmp(procFilePath("net/snmp"))
	if err != nil {
		return err
	}
	...
	tcpConnNum := snmp.TcpCurrEstab
	tcpRetrans := uint64(float64(snmp.TcpRetransSegs-c.lastTcpRetransSegs) / elapsedSec)
	tcpActiveOpens := uint32(float64(snmp.TcpActiveOpens-c.lastTcpActiveOpens) / elapsedSec)

	ch <- plugin.Metric{Name: tcpConnNumMetric, Value: float64(tcpConnNum)}.With(HostLabels...)
	ch <- plugin.Metric{Name: tcpRetransMetric, Value: float64(tcpRetrans)}.With(HostLabels...)
	ch <- plugin.Metric{Name: tcpActiveOpensMetric, Value: float64(tcpActiveOpens)}.With(HostLabels...)

	c.lastTcpRetransSegs = snmp.TcpRetransSegs
	c.lastTcpActiveOpens = snmp.TcpActiveOpens
